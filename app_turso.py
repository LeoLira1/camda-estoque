...
def build_principios_ativos_tab(df_mestre: pd.DataFrame, df_pa: pd.DataFrame):
    ...
    top20_pas = df_id.head(20)["principio_ativo"].tolist()
    pa_cores_prog = {pa: _PA_PALETTE[i % len(_PA_PALETTE)] for i, pa in enumerate(top20_pas)}

    _hoje_prog = datetime.now(tz=_BRT).date()
    _mes_ini   = date(_hoje_prog.year, _hoje_prog.month, 1)

    col_tipo_prog, col_di_prog, col_df_prog = st.columns([3, 2, 2])
    with col_tipo_prog:
        tipo_prog = st.radio(
            "Exibir",
    ...
    with col_di_prog:
        data_prog_ini = st.date_input("De", value=_mes_ini, key="pa_prog_di")
    with col_df_prog:
    ...
    def _prog_estoque() -> "pd.DataFrame":
        ...
        for produto, qtd, dia in (rows or []):
            ...
            if pa == "Não identificado" or pa not in top20_pas:
                continue
            litros = extrair_litros(str(produto))
        ...
        df = df.groupby(["dia", "principio_ativo"]).agg(volume=("volume", "sum")).reset_index()
        df["dia"] = pd.to_datetime(df["dia"])
        return df

    def _prog_reposicao() -> "pd.DataFrame":
        try:
            rows = get_db().execute("""
                SELECT produto, SUM(qtd_vendida) AS qtd, DATE(criado_em) AS dia
                  FROM reposicao_loja
                 WHERE DATE(criado_em) >= ? AND DATE(criado_em) <= ?
                 GROUP BY produto, DATE(criado_em)
                 ORDER BY dia
            """, (data_prog_ini.isoformat(), data_prog_fim.isoformat())).fetchall()
        except Exception:
            return pd.DataFrame()
    ...
    if df_pv.empty and df_pr.empty:
        ...
        for i, pa in enumerate(top20_pas):
            cor = pa_cores_prog[pa]
            ...
            if show_v and not df_pv.empty:
                ...
                if not _d.empty:
                    fig_prog.add_trace(go.Scatter(
                        ...
                        legendgroup=pa,
                        showlegend=True,
                        hovertemplate=f"<b>{pa}</b><br>%{{x|%d/%m}}: %{{y:.1f}}<extra></extra>",
                    ))
