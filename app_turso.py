...
(function() {{
  ...
  var dismissed   = ss.getItem('camda_cal_dismissed') === '1';
  var sameSession = ss.getItem('camda_cal_session')   === SESSION;

  var old = doc.getElementById('camda-cal-overlay');
  if (old) old.remove();
  var oldSt = doc.getElementById('camda-cal-style');
  if (oldSt) oldSt.remove();

  if (dismissed && sameSession) return;

  if (!sameSession) {{
    ss.removeItem('camda_cal_dismissed');
    ss.setItem('camda_cal_session', SESSION);
    ss.setItem('camda_cal_vy', TY);
    ss.setItem('camda_cal_vm', TM);
  }}

  var VY = parseInt(ss.getItem('camda_cal_vy') || TY);
  var VM = parseInt(ss.getItem('camda_cal_vm') || TM);

  var st_el = doc.createElement('style');
  st_el.id = 'camda-cal-style';
  st_el.textContent =
  ...
  overlay.id = 'camda-cal-overlay';
  overlay.innerHTML =
    '<div id="camda-cal-card">' +
      '<button id="camda-cal-close">&#x2715;</button>' +
      '<div id="camda-cal-bigday"></div>' +
      '<div id="camda-cal-monthname"></div>' +
      '<div id="camda-cal-yeardow"><span id="camda-cal-year"></span><span id="camda-cal-dow"></span></div>' +
      '<div class="camda-cal-nav">' +
        '<button class="camda-cal-navbtn" id="camda-cal-prev">&#x2039;</button>' +
        '<button class="camda-cal-navbtn" id="camda-cal-next">&#x203a;</button>' +
      '</div>' +
      '<div id="camda-cal-grid"></div>' +
    '</div>';
  ...
  function render() {{
    ...
    doc.getElementById('camda-cal-bigday').textContent = (VY===TY && VM===TM) ? TD : '';
    doc.getElementById('camda-cal-monthname').textContent = MESES[VM-1];
    doc.getElementById('camda-cal-year').textContent = VY;
    doc.getElementById('camda-cal-dow').textContent  = DIAS[dowIdx];
    ...
    doc.getElementById('camda-cal-grid').innerHTML = h;
  }}

  doc.getElementById('camda-cal-close').addEventListener('click', function() {{
    ss.setItem('camda_cal_dismissed', '1');
    ss.setItem('camda_cal_session', SESSION);
    overlay.remove();
    st_el.remove();
  }});
  doc.getElementById('camda-cal-prev').addEventListener('click', function() {{
    VM--; if (VM < 1) {{ VM = 12; VY--; }}
    ss.setItem('camda_cal_vy', VY);
    ss.setItem('camda_cal_vm', VM);
    render();
  }});
  doc.getElementById('camda-cal-next').addEventListener('click', function() {{
    VM++; if (VM > 12) {{ VM = 1; VY++; }}
    ss.setItem('camda_cal_vy', VY);
    ss.setItem('camda_cal_vm', VM);
    render();
...
def _get_connection():
    ...
    try:
        # Recria os triggers para garantir versão atualizada
        conn.execute("DROP TRIGGER IF EXISTS trig_contagem_to_cicli")
        conn.execute("DROP TRIGGER IF EXISTS trig_contagem_status_ciclo")
        conn.execute("""
            CREATE TRIGGER trig_contagem_to_cicli
            ...
            BEGIN
                ...
                SELECT
                    ...
                    CASE NEW.status
                        ...
                        ELSE CAST(COALESCE(em.qtd_fisica, NEW.qtd_estoque) AS REAL)
                    END,
                    CASE NEW.status
                        WHEN 'certa' THEN 0.0
                        ELSE CAST(COALESCE(em.qtd_fisica, NEW.qtd_estoque) AS REAL) - CAST(NEW.qtd_estoque AS REAL)
                    END,
                    datetime('now', '-3 hours'),
                    COALESCE(NEW.motivo, '')
                FROM estoque_mestre em
                WHERE em.codigo = NEW.codigo;
            END
        """)
        conn.execute("""
            CREATE TRIGGER trig_contagem_status_ciclo
            AFTER UPDATE OF status ON contagem_itens
            WHEN NEW.status IN ('certa', 'divergencia') AND OLD.status != NEW.status
            BEGIN
...
def listar_fotos_avaria(avaria_id: int) -> list[dict]:
    """Retorna lista de fotos de uma avaria, ordenadas por id."""
    try:
        rows = get_db().execute(
            "SELECT id, avaria_id, foto_base64, registrado_em "
            "FROM avaria_fotos WHERE avaria_id = ? ORDER BY id",
            (avaria_id,)
        ).fetchall()
        return [{"id": r[0], "avaria_id": r[1], "foto_base64": r[2], "registrado_em": r[3]}
                for r in rows]
    except Exception:
        return []
...
def adicionar_foto_avaria(avaria_id: int, foto_bytes: bytes) -> bool:
    ...
    try:
        ...
        conn = get_db()
        conn.execute(
            "INSERT INTO avaria_fotos (avaria_id, foto_base64, registrado_em) VALUES (?, ?, ?)",
            (avaria_id, foto_b64, now)
        )
        conn.commit()
        sync_db()
        return True
...
def remover_foto_item(foto_id: int):
    """Remove uma foto individual da tabela avaria_fotos pelo id."""
    try:
        conn = get_db()
        conn.execute("DELETE FROM avaria_fotos WHERE id = ?", (foto_id,))
        conn.commit()
        sync_db()
    except Exception as e:
...
# Tolerância para considerar quantidade vendida "similar" ao delta da divergência (50%)
_QTD_MATCH_TOLERANCIA = 0.50
  ...
  options:{
    ...
    with t7:
        ...
        if df_av.empty:
            ...
            st.markdown(
                f'<div class="av-grid-open-label">'
                f'Avarias abertas <span class="av-count-badge">{len(df_av)}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
            ...
            av_list = list(df_av.iterrows())
            for pair_start in range(0, len(av_list), 2):
                pair = av_list[pair_start:pair_start + 2]
                grid_cols = st.columns(2, gap="medium")

                for col_idx, (_, av) in enumerate(pair):
                    with grid_cols[col_idx]:
                        ...
                        card_cls = "av-card" if is_aberta else "av-card fechada"
                        ...
                        if galoes_html or _foto_cards_html:
                            ...
                            _galoes_container = (
                                ...
                                'align-items:flex-end;padding:4px 0 8px;">'
                                + galoes_html
                                + _foto_cards_html
                                + '</div>'
                            )
                        ...
                        if unidades:
                            ...
                            st.markdown(
                                f'<div class="av-stats-bar">'
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Restante</span>'
                                f'<span class="av-stat-val">{total_rest:.1f} L</span>'
                                f'</div>'
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Perdido</span>'
                                f'<span class="av-stat-val perdido">{total_perd:.1f} L</span>'
                                f'</div>'
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Baldes</span>'
                                f'<span class="av-stat-val baldes">{len(unidades)}</span>'
                                f'</div>'
                                f'</div>',
                                unsafe_allow_html=True
                            )
                            ...
                            st.markdown(
                                ...
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Restante</span>'
                                f'<span class="av-stat-val">—</span>'
                                f'</div>'
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Perdido</span>'
                                f'<span class="av-stat-val perdido">—</span>'
                                f'</div>'
                                f'<div class="av-stat-item">'
                                f'<span class="av-stat-lbl">Unid.</span>'
                                f'<span class="av-stat-val baldes">{int(av.get("qtd_avariada", 0))}</span>'
                                f'</div>'
                                f'</div>',
                                unsafe_allow_html=True
                            )
