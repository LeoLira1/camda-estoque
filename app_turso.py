...
def build_radar_tab(df_mestre: pd.DataFrame, df_pa: pd.DataFrame):
    """Aba de constelação animada — tela de descanso (F11)."""
    ...
    # ── Serializa dados para o JS ─────────────────────────────────────────────
    max_qtd = max(int(df_mestre["qtd_sistema"].max()), 1)
    stars_data = []
    for _, row in df_mestre.iterrows():
        stars_data.append({
            "nome":      str(row["produto"])[:45],
            "categoria": str(row["categoria"]),
            "qtd":       max(0, int(row["qtd_sistema"]) if pd.notna(row["qtd_sistema"]) else 0),
            "status":    str(row["status"])  if pd.notna(row["status"])  else "ok",
        })

    pa_groups: dict = {}
    if not df_pa.empty:
        for _, row in df_pa.iterrows():
            pa   = str(row["principio_ativo"]).strip()
            prod = str(row["produto"]).strip()
            if pa and pa.lower() not in ("não identificado", "nao identificado", ""):
                pa_groups.setdefault(pa, []).append(prod)
    pa_connections = {k: v for k, v in pa_groups.items() if len(v) >= 2}

    payload = _json.dumps({"stars": stars_data, "pa": pa_connections, "maxQtd": max_qtd},
                          ensure_ascii=False)
...
#tt{
  ...
  background:rgba(10,15,26,0.88);border:1px solid rgba(255,255,255,0.08);
  border-radius:10px;padding:9px 13px;color:#e0e6ed;font-size:12px;
  font-family:'DM Sans',sans-serif;max-width:220px;backdrop-filter:blur(12px);
  z-index:99;box-shadow:0 4px 24px rgba(0,0,0,0.5);
}
#tt b{display:block;font-size:13px;margin-bottom:2px}
#tt small{display:block;color:#64748b;font-size:10px;margin-bottom:5px}
#tt span{font-family:monospace;font-size:11px}
#leg{
  position:absolute;bottom:12px;left:50%;transform:translateX(-50%);
  display:flex;gap:18px;background:rgba(10,15,26,0.7);
...
<body>
<canvas id="c"></canvas>
<div id="tt"><b id="tn"></b><small id="tc"></small><span id="tq"></span></div>
<div id="leg">
  <div class="li"><div class="ld" style="background:#00d68f;box-shadow:0 0 5px #00d68f"></div>OK</div>
  <div class="li"><div class="ld" style="background:#ff4757;box-shadow:0 0 5px #ff4757"></div>Falta</div>
  <div class="li"><div class="ld" style="background:#ffa502;box-shadow:0 0 5px #ffa502"></div>Sobra</div>
  <div class="li" style="opacity:.6"><div class="ld" style="background:#a55eea"></div>mesmo P.A.</div>
</div>
<script>
...
const CLR = {ok:'#00d68f', falta:'#ff4757', sobra:'#ffa502'};
...
function buildLayout(){
  ...
  CONNS=[];
  Object.entries(DATA.pa).forEach(([pa,prods])=>{
    const ids = prods.map(p=>idx[p.trim().toUpperCase()]).filter(i=>i!==undefined);
    for(let a=0;a<ids.length;a++)
      for(let b=a+1;b<ids.length;b++)
        CONNS.push({i:ids[a],j:ids[b],pa,ph:Math.random()*Math.PI*2});
  });
  ...
  for(let i=0;i<180;i++){
    BGSTARS.push({x:Math.random()*W,y:Math.random()*H,
                  r:Math.random()*0.7,a:0.06+Math.random()*0.22,
                  ph:Math.random()*Math.PI*2,spd:0.15+Math.random()*0.4});
  }
}
buildLayout();

// ── animation ────────────────────────────────────────────────────────────
let rot=0, t=0, mx=-9999, my=-9999, hov=-1;

canvas.addEventListener('mousemove', e=>{
  const r=canvas.getBoundingClientRect();
  mx=e.clientX-r.left; my=e.clientY-r.top;
});
canvas.addEventListener('mouseleave', ()=>{ mx=-9999; my=-9999; hov=-1; tt.style.display='none'; });

function getPos(s){
  const co=Math.cos(rot), si=Math.sin(rot);
  return { x: canvas.width/2  + s.bx*co - s.by*si,
           y: canvas.height/2 + s.bx*si + s.by*co };
}
...
function frame(){
  ...
  // bg micro-stars (twinkle in place)
  BGSTARS.forEach(s=>{
  ...
  // PA connection lines
  CONNS.forEach(c=>{
    if(c.i>=STARS.length||c.j>=STARS.length) return;
    const a=getPos(STARS[c.i]), b=getPos(STARS[c.j]);
    const pulse=0.05+0.045*Math.sin(t*0.55+c.ph);
    const cA=CLR[STARS[c.i].status]||'#94a3b8';
    const cB=CLR[STARS[c.j].status]||'#94a3b8';
    const g=ctx.createLinearGradient(a.x,a.y,b.x,b.y);
    g.addColorStop(0,  `rgba(${hexRgb(cA)},${pulse})`);
    g.addColorStop(0.5,`rgba(165,94,234,${pulse*0.6})`);
    g.addColorStop(1,  `rgba(${hexRgb(cB)},${pulse})`);
  ...
  STARS.forEach((s,i)=>{
    ...
    if(d<Math.max(s.br*5,10)&&d<minD){ minD=d; hov=i; }
  });
  ...
  STARS.forEach((s,i)=>{
    if(s.isLabel) return;
    const p   = getPos(s);
    const tw  = 0.72+0.28*Math.sin(t*s.spd+s.phase);
    const col = CLR[s.status]||'#94a3b8';
    const r   = s.br*tw;
    const iH  = hov===i;
    const gr  = r*(iH?7:4.5);

    const grd=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,gr);
    grd.addColorStop(0,   `rgba(${hexRgb(col)},0.38)`);
    grd.addColorStop(0.35,`rgba(${hexRgb(col)},0.12)`);
    grd.addColorStop(1,   `rgba(${hexRgb(col)},0)`);
    ctx.beginPath(); ctx.arc(p.x,p.y,gr,0,Math.PI*2);
    ctx.fillStyle=grd; ctx.fill();

    ctx.beginPath(); ctx.arc(p.x,p.y,r*(iH?1.7:1),0,Math.PI*2);
  ...
  // tooltip
  if(hov>=0&&hov<STARS.length){
    const s=STARS[hov], p=getPos(s);
    const col=CLR[s.status]||'#94a3b8';
    tn.textContent=s.nome; tn.style.color=col;
    tc.textContent=s.categoria;
    tq.textContent=`Estoque: ${s.qtd} un · ${s.status.toUpperCase()}`;
    tq.style.color=col;
    tt.style.display='block';
    tt.style.left=Math.min(p.x+16,W-230)+'px';
    tt.style.top =Math.max(p.y-44,8)+'px';
...
frame();
    ...
    html = _HTML_TMPL.replace("__PAYLOAD__", payload)
    _comp.html(html, height=660, scrolling=False)

    # ── Gráficos analíticos (opcionais) ──────────────────────────────────────
    with st.expander("📊 Ver gráficos analíticos", expanded=False):
        ...
        with col_p:
            df_vendas_r2 = df_vendas_r if "df_vendas_r" in dir() else get_vendas_historico()
            if not df_pa.empty and not df_vendas_r2.empty:
                df_pa_m = df_pa[["produto", "principio_ativo"]].rename(columns={"principio_ativo": "pa"})
                df_v_pa = df_vendas_r2[["produto", "qtd_vendida"]].merge(df_pa_m, on="produto", how="inner")
                ...
                for pa, sub in df_pa_grp.groupby("pa"):
                    ...
                    if (mn > 0 and mx / mn >= 3) or (mn == 0 and mx > 0):
                        pas_disp.append(pa)
                    ...
                    for pa in top_pas:
                        ...
                        for _, row in sub.iterrows():
                            ...
                            fig_pa.add_trace(go.Bar(
                                name=prod_label, x=[pa_label], y=[row["qtd_vendida"]],
                                marker_color=cor, showlegend=False,
                                text=prod_label, textposition="inside", textfont=dict(size=9),
                                hovertemplate=f"<b>{row['produto']}</b><br>P.A.: {pa}<br>Vendas: {row['qtd_vendida']}<extra></extra>",
                            ))

                    fig_pa.update_layout(
                        **_PLOTLY_LAYOUT, barmode="group", height=350,
                        title=dict(text="🔗 Oportunidades por Princípio Ativo  🟢 vende bem · 🔴 esquecido",
