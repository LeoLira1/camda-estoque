...
CATEGORIAS_EXCLUIDAS_REPOSICAO = frozenset({
    ...
    "SEMENTES", "MEDICAMENTOS", "MEDICAMENTOS VETERINÁRIOS", "MEDICAMENTOS VETERINARIOS",
    "VACINA AFTOSA", "VACINAS DIVERSAS/SOROS", "ANTIBIOTICOS/ANTI-INFLAMATORIO",
    "ANESTESICO/ANALGESICO/DIURETIC", "ANTITOXICOS", "VERMIFUGOS",
    "MOSQUICIDA/CARRAPATICIDA/BERNI", "UNGUENTOS/POMADAS", "HOMEOPATICO",
    "HORMONIOS LEITEIROS", "TONICO MINERAL/VITAMINAS", "REPRODUCAO ANIMAL",
    "REPRODUTORES", "IDENTIFICACAO ANIMAL", "INOCULANTES P/ SILAGEM",
    "DIETA ANIMAL", "RATICIDAS",
})
...
def load_principios_ativos_from_excel(filepath: str) -> list:
    try:
        ...
        for c in df.columns:
            ...
            if cu == "PRODUTO": col_prod = c
            elif "PRINC" in cu or "ATIVO" in cu: col_pa = c
            elif "CATEG" in cu: col_cat = c
        if not col_prod or not col_pa: return []
        records = []
...
def search_by_praga_agrofit(search_term: str, df_estoque: pd.DataFrame) -> set:
    ...
    os nomes dos produtos do estoque local que correspondem.
    Modificado para evitar falsos positivos exigindo match exato de marca
    ou similaridade super alta.
    """
    import traceback
    ...
    try:
        ...
        # Otimização: limpar marcas uma única vez para não repetir no loop
        marcas_limpas = []
        for m in marcas_agrofit:
            m_cl = re.sub(r"[^A-Z0-9 ]", "", str(m).upper()).strip()
            if m_cl: marcas_limpas.append(m_cl)

        for prod in produtos_estoque:
            prod_str = str(prod).upper()
            prod_cl = re.sub(r"[^A-Z0-9 ]", "", prod_str).strip()
            
            match_found = False
            # 1. Busca por contenção direta (Muito mais seguro e não aceita palavras aleatórias)
            for marca in marcas_limpas:
                if len(marca) <= 3:
                    # Marcas com 3 letras ou menos (ex: "DIP") precisam ser palavra exata para não casar com sílabas soltas
                    if re.search(rf'\b{marca}\b', prod_cl):
                        matches.add(prod_str)
                        match_found = True
                        break
                else:
                    # Marcas maiores (ex: "ENGEO PLENO S") basta estarem contidas na string ("ENGEO PLENO S 250ML")
                    if marca in prod_cl:
                        matches.add(prod_str)
                        match_found = True
                        break
            
            # 2. Se não achou por contenção direta, tenta fuzzy apenas se a aproximação for gigante (>= 0.85)
            if not match_found:
                for marca in marcas_limpas:
                    score = _similaridade(prod_cl, marca)
                    if score >= 0.85:
                        matches.add(prod_str)
                        break
...
def parse_annotation(nota: str, qtd_sistema: int) -> tuple:
    if not nota: return (qtd_sistema, 0, "", "ok")
    text = nota.strip()
...
def parse_estoque_format(df_raw: pd.DataFrame) -> tuple:
    ...
    for c in df.columns:
        cu = c.upper().strip()
        if cu == "PRODUTO" and "produto" not in col_map: col_map["produto"] = c
        elif ("QUANTIDADE" in cu or cu == "QTD") and "qtd" not in col_map: col_map["qtd"] = c
        elif cu in ("CÓDIGO", "CODIGO", "COD") and "codigo" not in col_map: col_map["codigo"] = c
        elif ("OBS" in cu or "NOTA" in cu or "DIFEREN" in cu or "ANOTA" in cu) and "nota" not in col_map: col_map["nota"] = c
    ...
    for _, row in df.iterrows():
        ...
        if col_cod:
            ...
            if codigo.upper() in ("NAN", "NONE", ""): codigo = ""
        if not codigo:
        ...
        if col_nota:
            ...
            if nota_raw.upper() in ("NAN", "NONE"): nota_raw = ""
            if _RE_ONLY_NUMBER.match(nota_raw): nota_raw = ""
...
def parse_vendas_format(df_raw: pd.DataFrame) -> tuple:
    header_idx = _find_header(df_raw, lambda vals: "PRODUTO" in vals and ("QTDD" in " ".join(vals) or "VENDIDA" in " ".join(vals)))
    if header_idx is None:
    ...
    for c in df.columns:
        cu = c.upper().strip()
        if "GRUPO" in cu and not col_grupo: col_grupo = c
        elif cu == "PRODUTO" and not col_produto: col_produto = c
        elif "VENDIDA" in cu and not col_qtd_vendida: col_qtd_vendida = c
        elif "ESTOQUE" in cu and not col_qtd_estoque: col_qtd_estoque = c
        elif ("OBS" in cu or "NOTA" in cu or "ANOTA" in cu) and not col_nota: col_nota = c
    ...
    if not col_produto: return (False, f"Coluna 'PRODUTO' não encontrada.")
    if not col_qtd_estoque and not col_qtd_vendida: return (False, "Nenhuma coluna de quantidade encontrada.")
    ...
    for _, row in df.iterrows():
        ...
        if not raw_prod or raw_prod.upper() in ("NAN", "NONE", "ROLLUP"): continue

        m = _RE_COD_PROD.match(raw_prod)
        if m: codigo, produto = m.group(1).strip(), m.group(2).strip()
        else:
        ...
        if col_qtd_estoque:
            try:
                ...
                if pd.notna(val): qtd_sistema = int(float(val))
            except (ValueError, TypeError): pass
        if col_qtd_vendida:
            try:
                val = row.get(col_qtd_vendida)
                if pd.notna(val): qtd_vendida_val = int(float(val))
            except (ValueError, TypeError): pass
        ...
        if categoria in ("OUTROS", ""): categoria = classify_product(produto)
...
def read_excel_to_records(uploaded_file) -> tuple:
    ...
    if fmt == "vendas": return parse_vendas_format(df_raw)
    elif fmt == "estoque":
        ...
        if ok2: return (ok2, result2, zerados2)
...
def upload_parcial(records: list, zerados: list = None) -> tuple:
    try:
        ...
        if update_data: parts.append(f"{len(update_data)} atualizados")
        if novos_data: parts.append(f"{len(novos_data)} novos")
        if n_div: parts.append(f"{n_div} divergências")
        if n_repo: parts.append(f"🏪 {n_repo} para repor na loja")
...
def _detectar_reposicao_batch(records: list, conn, now: str) -> int:
    ...
    for r in records:
        ...
        if cat in CATEGORIAS_EXCLUIDAS_REPOSICAO: continue
        if r["codigo"] in pending: continue
...
def build_css_treemap(df: pd.DataFrame, filter_cat: str = "TODOS", avarias_map: dict = None) -> str:
    if df.empty: return '<div style="color:#64748b;text-align:center;padding:40px;">Nenhum produto para exibir</div>'
    if filter_cat != "TODOS": df = df[df["categoria"] == filter_cat]
    if df.empty: return '<div style="color:#64748b;text-align:center;padding:40px;">Nenhum produto nesta categoria</div>'

    if avarias_map is None: avarias_map = {}
    categories = {}
    for _, row in df.iterrows():
    ...
    for cat in sort_categorias(list(categories.keys())):
        ...
        for r in sorted(rows, key=lambda x: str(x["produto"])):
            ...
            if diff == 0: bg, txt, info = "#00d68f", "#0a2e1a", str(qs)
            elif diff < 0: bg, txt, info = "#ff4757", "#fff", f"{qf} (F {abs(diff)})"
            else: bg, txt, info = "#ffa502", "#fff", f"{qf} (S {diff})"
...
def save_vendas_historico(records: list, grupo_map: dict, zerados: list = None, is_mestre: bool = False):
    try:
        ...
        if is_mestre:
            ...
            if zerados:
                for z in zerados:
                    if isinstance(z, dict): codigos_update.append(z["codigo"])
            if codigos_update:
...
def build_vendas_tab(df_vendas: pd.DataFrame):
    if df_vendas.empty:
        st.info("📊 Nenhum dado de vendas carregado ainda.")
        return
    ...
    with vt1:
        ...
        fig_bar.add_trace(go.Bar(
            y=df_top["grupo"], x=df_top["qtd_vendida"], orientation="h",
            marker=dict(color=[_get_color(g) for g in df_top["grupo"]], line=dict(width=0), cornerradius=4),
            text=df_top["qtd_vendida"].apply(lambda v: f"{v:,.0f}".replace(",", ".")),
            textposition="outside", textfont=dict(size=10, color="#94a3b8"),
            hovertemplate="<b>%{y}</b><br>Vendido: %{x:,.0f}<extra></extra>",
        ))
        fig_bar.update_layout(
            **_PLOTLY_LAYOUT, title=dict(text="Quantidade Vendida por Grupo", font=dict(size=14, color="#e0e6ed")),
            height=max(350, top_n * 32), yaxis=dict(autorange="reversed", gridcolor="rgba(0,0,0,0)"),
            xaxis=dict(gridcolor="#1e293b", title=None), showlegend=False,
        )
        ...
        with c1:
            fig_pie = go.Figure(go.Pie(
                labels=df_top["grupo"].head(8), values=df_top["qtd_vendida"].head(8), hole=0.55,
                marker=dict(colors=[_get_color(g) for g in df_top["grupo"].head(8)]),
                textinfo="percent", textfont=dict(size=10, color="#e0e6ed"),
                hovertemplate="<b>%{label}</b><br>%{value:,.0f} un<br>%{percent}<extra></extra>",
            ))
            fig_pie.update_layout(
                **_PLOTLY_LAYOUT, title=dict(text="Distribuição % Vendas", font=dict(size=13, color="#94a3b8")),
                height=320, showlegend=True, legend=dict(font=dict(size=9), orientation="h", y=-0.15),
            )
        ...
        with c2:
            fig_vs = go.Figure()
            df8 = df_top.head(8)
            fig_vs.add_trace(go.Bar(name="Vendido", x=df8["grupo"], y=df8["qtd_vendida"], marker=dict(color="#00d68f", cornerradius=3, opacity=0.85)))
            fig_vs.add_trace(go.Bar(name="Estoque", x=df8["grupo"], y=df8["qtd_estoque"], marker=dict(color="#3b82f6", cornerradius=3, opacity=0.5)))
            fig_vs.update_layout(
                **_PLOTLY_LAYOUT, barmode="group", title=dict(text="Vendido vs Estoque", font=dict(size=13, color="#94a3b8")),
                height=320, xaxis=dict(tickangle=-35, tickfont=dict(size=8), gridcolor="rgba(0,0,0,0)"),
    ...
    with vt2:
        ...
        df_crit = df_vendas[(df_vendas["qtd_estoque"] > 0) & (df_vendas["qtd_estoque"] < df_vendas["qtd_vendida"] * 0.5) & (df_vendas["qtd_vendida"] > 10)].copy()
        ...
        with kc1: st.markdown(f"""<div class="stat-card"><div class="stat-value red">{len(df_zero)}</div><div class="stat-label">💀 Zerados c/ vendas</div></div>""", unsafe_allow_html=True)
        with kc2: st.markdown(f"""<div class="stat-card"><div class="stat-value amber">{len(df_crit)}</div><div class="stat-label">🔥 Crítico &lt;50%</div></div>""", unsafe_allow_html=True)
        with kc3: st.markdown(f"""<div class="stat-card"><div class="stat-value purple">{len(df_zero)+len(df_crit)}</div><div class="stat-label">⚡ Total Alertas</div></div>""", unsafe_allow_html=True)

        df_alerta = pd.concat([df_zero.head(15).assign(nivel="ZERADO", dias_cobertura=0.0), df_crit.head(15).assign(nivel="CRÍTICO")]).sort_values(["nivel", "dias_cobertura"], ascending=[True, True], key=lambda col: col.map({"ZERADO": 0, "CRÍTICO": 1}) if col.name == "nivel" else col).head(25)
        ...
        if not df_alerta.empty:
            ...
            fig_alert.update_layout(
                **_PLOTLY_LAYOUT, title=dict(text="🚨 Produtos com Estoque Crítico vs Vendas", font=dict(size=13)),
                height=max(400, len(df_alerta) * 28), yaxis=dict(autorange="reversed", gridcolor="rgba(0,0,0,0)", tickfont=dict(size=9)),
                xaxis=dict(title="Qtd Vendida", gridcolor="#1e293b"), showlegend=False,
            )
    ...
    with vt3:
        st.caption("Estimativa de dias até zerar estoque (baseado nos últimos 47 dias de vendas)")
        ...
        if not df_burn.empty:
            colors_burn = ["#ff4757" if d < 15 else "#ffa502" if d < 30 else "#00d68f" for d in df_burn["dias_estoque"]]
            fig_burn = go.Figure()
            ...
            fig_burn.add_shape(type="line", x0=15, x1=15, y0=-0.5, y1=len(df_burn)-0.5, line=dict(dash="dash", color="#ff4757", width=1), yref="y")
            fig_burn.add_annotation(x=15, y=-0.5, text="Urgente", showarrow=False, font=dict(size=9, color="#ff4757"), yshift=-12)
            fig_burn.add_shape(type="line", x0=30, x1=30, y0=-0.5, y1=len(df_burn)-0.5, line=dict(dash="dash", color="#ffa502", width=1), yref="y")
            fig_burn.add_annotation(x=30, y=-0.5, text="Atenção", showarrow=False, font=dict(size=9, color="#ffa502"), yshift=-12)
            fig_burn.update_layout(
                **_PLOTLY_LAYOUT, title=dict(text="🔥 Dias de Estoque Restante por Grupo", font=dict(size=14)),
                height=max(350, len(df_burn) * 35), yaxis=dict(autorange="reversed", gridcolor="rgba(0,0,0,0)"),
                xaxis=dict(title="Dias", gridcolor="#1e293b"), showlegend=False,
            )
            st.plotly_chart(fig_burn, width='stretch', config={"displayModeBar": False})

            urgentes = df_burn[df_burn["dias_estoque"] < 15]["grupo"].tolist()
            if urgentes: st.error(f"⚡ **Reposição urgente** (<15 dias): {', '.join(urgentes)}")
            atencao = df_burn[(df_burn["dias_estoque"] >= 15) & (df_burn["dias_estoque"] < 30)]["grupo"].tolist()
            if atencao: st.warning(f"⚠️ **Atenção** (15-30 dias): {', '.join(atencao)}")
    ...
    with vt4:
        ...
        if grupo_sel != "TODOS": df_prod = df_prod[df_prod["grupo"] == grupo_sel]
        ...
        if not df_top_prod.empty:
            ...
            fig_top.add_trace(go.Bar(
                ...
                marker=dict(color=[_get_color(g) for g in df_top_prod["grupo"]], cornerradius=4),
                text=df_top_prod["qtd_vendida"].apply(lambda v: f"{v:,.0f}".replace(",", ".")),
                textposition="outside", textfont=dict(size=9, color="#94a3b8"),
                hovertemplate="<b>%{y}</b><br>Vendido: %{x:,.0f}<br><extra></extra>",
            ))
            fig_top.update_layout(
                **_PLOTLY_LAYOUT, title=dict(text=f"🏆 Top 15 Produtos — {grupo_sel}", font=dict(size=14)),
                height=max(380, len(df_top_prod) * 30), yaxis=dict(autorange="reversed", gridcolor="rgba(0,0,0,0)", tickfont=dict(size=9)),
                xaxis=dict(title="Qtd Vendida", gridcolor="#1e293b"), showlegend=False,
            )
            ...
            for g in df_prod["grupo"].unique():
                ...
                fig_scatter.add_trace(go.Scatter(
                    x=dg["qtd_vendida"], y=dg["qtd_estoque"], mode="markers", name=g[:15],
                    marker=dict(color=_get_color(g), size=8, opacity=0.7, line=dict(width=1, color="#0a0f1a")),
            ...
            fig_scatter.add_trace(go.Scatter(x=[0, max_val], y=[0, max_val], mode="lines", line=dict(dash="dash", color="#64748b", width=1), name="Equilíbrio", showlegend=True))
            fig_scatter.update_layout(
                **_PLOTLY_LAYOUT, title=dict(text="Vendido × Estoque", font=dict(size=12, color="#94a3b8")),
                height=380, xaxis=dict(title="Qtd Vendida", gridcolor="#1e293b"),
                yaxis=dict(title="Qtd Estoque", gridcolor="#1e293b"), legend=dict(font=dict(size=8), orientation="h", y=-0.2),
            )
...
with st.expander("📤 Upload de Planilha", expanded=not has_mestre):
    ...
    upload_mode = st.radio(
        "Tipo de Upload", [opcao_mestre, opcao_parcial],
        index=0 if not has_mestre else 1, horizontal=True, label_visibility="collapsed",
    )
    ...
    if uploaded:
        ...
        if ok:
            ...
            with st.expander("👁️ Preview", expanded=False):
                ...
                if n_div: st.warning(f"⚠️ {n_div} divergência(s)")
                if zerados: st.info(f"🗑️ {len(zerados)} produto(s) com estoque zerado serão removidos do mestre")
                st.dataframe(
            ...
            if st.button("🚀 Processar", type="primary"):
                with st.spinner("Processando..."):
                    if is_mestre_upload: ok_up, msg = upload_mestre(records)
                    else: ok_up, msg = upload_parcial(records, zerados)

                if ok_up:
                    st.success(msg)
                    save_vendas_historico(records, _GRUPO_MAP, zerados, is_mestre=is_mestre_upload)
                    if _using_cloud: st.info("☁️ Sincronizado.")
                    st.session_state.processed_file = None
                    st.rerun()
                else:
                    st.error(msg)
...
if has_mestre:
    ...
    if has_pa and _has_agrofit_token: search_placeholder = "Nome, Código, Princípio Ativo ou Praga..."
    elif has_pa: search_placeholder = "Nome, Código ou Princípio Ativo..."
    elif _has_agrofit_token: search_placeholder = "Nome, Código ou Praga (via Agrofit)..."
    else: search_placeholder = "Nome ou Código..."
    ...
    if search_term:
        ...
        if _has_agrofit_token and not mask.any():
            ...
            with st.expander("🔬 Diagnóstico detalhado Agrofit", expanded=True):
                if _dbg.get("erro"): st.error(f"Erro na API: {_dbg['erro']}")
                if _dbg.get("raw_keys_exemplo"): st.write(f"**🔑 Campos da API (raw):** `{_dbg['raw_keys_exemplo']}`")
                st.write(f"**Passo 1 — busca direta** `praga_nome_comum={search_term}`: {_dbg.get('produtos_direto', '?')} produto(s) → marcas: `{_dbg.get('marcas_direto', [])[:5]}`")
                pragas_norm = _dbg.get("pragas_comuns_encontradas", [])
                st.write(f"**Passo 2 — pragas conhecidas** `/search/pragas-nomes-comuns?q={search_term}`: `{pragas_norm}`")
                ...
                st.write(f"**Match exato ou fuzzy(≥0.85) no estoque:** {len(praga_produtos)} produto(s) → `{list(praga_produtos)[:5]}`")
        ...
        if pa_match_info: st.caption(pa_match_info)
        if praga_match_info: st.caption(praga_match_info)
        if not mask.any(): st.warning(f"Nenhum produto encontrado para **'{search_term}'**.")
    ...
    with t2:
        ...
        if df_div.empty: st.info("Nenhuma divergência.")
        else:
            st.dataframe(df_div[["codigo", "produto", "categoria", "qtd_sistema", "qtd_fisica", "diferenca", "nota", "ultima_contagem"]], hide_index=True, width='stretch')

    with t3:
        if df_reposicao.empty: st.success("Nenhum produto pendente de reposição! 🎉")
        else:
            st.caption(f"{n_repor} produto(s) para repor. Itens somem após 7 dias ou quando marcados.")
            for _, item in df_reposicao.iterrows():
                try: dias = (datetime.now() - datetime.strptime(item["criado_em"], "%Y-%m-%d %H:%M:%S")).days
                except: dias = 0
    ...
    with t5:
        ...
        if df_hist.empty: st.info("Nenhum upload registrado.")
        else: st.dataframe(df_hist, hide_index=True, width='stretch')
    ...
    with t6:
        ...
        with st.expander("➕  Registrar nova pendência", expanded=False):
            foto = st.file_uploader("Foto da via cega", type=["jpg", "jpeg", "png", "webp"], label_visibility="collapsed", key="pend_foto_upload")
            if foto is not None:
                img_bytes = foto.read()
                ...
                with col_ok:
                    if st.button("✅ Salvar pendência", width='stretch', type="primary", key="pend_salvar"):
                        ...
                        st.rerun()
                with col_cancel:
                    if st.button("✖ Cancelar", width='stretch', key="pend_cancelar"): st.rerun()

        st.divider()
        pendencias = listar_pendencias()
        if not pendencias:
            st.markdown("""<div style="text-align:center;padding:40px 20px;color:rgba(255,255,255,0.3);"><div style="font-size:2.5rem;">✅</div><div>Nenhuma pendência no momento</div></div>""", unsafe_allow_html=True)
        else:
            ...
            for pid, foto_b64, data_reg in pendencias:
                ...
                if dias <= 1: card_class, badge_class, badge_txt = "pend-card", "badge-verde", "Hoje" if dias == 0 else "1 dia"
                elif dias == 2: card_class, badge_class, badge_txt = "pend-card alerta-amarelo", "badge-amarelo", "⚠️ 2 dias — entregar hoje!"
                else: card_class, badge_class, badge_txt = "pend-card alerta-vermelho", "badge-vermelho", f"🚨 {dias} dias — VENCIDO!"

                st.markdown(f'<div class="{card_class}"><span class="badge-dias {badge_class}">{badge_txt}</span><div class="pend-data">Registrado em: {data_reg}</div>', unsafe_allow_html=True)
                try: st.image(base64.b64decode(foto_b64), width='stretch')
    ...
    with t7:
        ...
        with st.expander("➕ Registrar nova avaria", expanded=False):
            ...
            if df_estoque_av.empty: st.info("Nenhum produto no estoque.")
            else:
                busca_av = st.text_input("🔍 Buscar produto", placeholder="Nome ou código...", key="av_busca")
                df_filtrado_av = df_estoque_av
                if busca_av:
                    df_filtrado_av = df_estoque_av[df_estoque_av["produto"].str.contains(busca_av, case=False, na=False) | df_estoque_av["codigo"].str.contains(busca_av, case=False, na=False)]
                if df_filtrado_av.empty: st.warning("Nenhum produto encontrado.")
                else:
                    ...
                    row_sel = df_filtrado_av.iloc[opcoes.index(sel)]
                    qtd_av = st.number_input("Qtd avariada", min_value=1, max_value=int(row_sel["qtd_sistema"]) if int(row_sel["qtd_sistema"]) > 0 else 9999, value=1, step=1, key="av_qtd")
                    motivo_av = st.text_area("Motivo / descrição", key="av_motivo", height=80)
                    if st.button("🔴 Registrar Avaria", type="primary", key="av_registrar"):
                        if not motivo_av.strip(): st.error("Descreva o motivo.")
                        else:
                            if registrar_avaria(row_sel["codigo"], row_sel["produto"], int(qtd_av), motivo_av.strip()):
                                st.success(f"✅ Avaria registrada.")
                                st.rerun()

        st.divider()
        col_f1, col_f2 = st.columns([2, 1])
        with col_f2: mostrar_resolvidas = st.toggle("Mostrar resolvidas", value=False, key="av_mostrar_resolvidas")

        df_av = listar_avarias(apenas_abertas=not mostrar_resolvidas)
        if df_av.empty: st.markdown("""<div style="text-align:center;padding:40px 20px;color:rgba(255,255,255,0.3);"><div style="font-size:2.5rem;">✅</div><div>Nenhuma avaria registrada</div></div>""", unsafe_allow_html=True)
        else:
            m1, m2, m3 = st.columns(3)
            m1.metric("Total", len(df_av))
            m2.metric("🔴 Abertas", (df_av["status"] == "aberto").sum())
            m3.metric("✅ Resolvidas", (df_av["status"] == "resolvido").sum())
            st.markdown("---")
            ...
            for _, av in df_av.iterrows():
                ...
                try: tempo_av = "hoje" if (datetime.now() - datetime.strptime(av["registrado_em"], "%Y-%m-%d %H:%M:%S")).days == 0 else f"{(datetime.now() - datetime.strptime(av['registrado_em'], '%Y-%m-%d %H:%M:%S')).days}d atrás"
                except: tempo_av = av["registrado_em"]

                st.markdown(f'<div class="{card_cls}"><span class="av-badge {badge_cls}">{badge_txt}</span>', unsafe_allow_html=True)
                col_info_av, col_btns_av = st.columns([5, 1])
                with col_info_av:
                    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:flex-start;"><div><div style="color:#e0e6ed;font-weight:700;font-size:0.9rem;">{av["produto"]}</div><div style="margin-top:4px;display:flex;gap:12px;flex-wrap:wrap;"><span style="color:#64748b;font-size:0.65rem;">Cod: <b style="color:#94a3b8;">{av["codigo"]}</b></span><span style="color:#ff4757;font-size:0.7rem;font-weight:700;">Qtd: {int(av["qtd_avariada"])}</span></div><div style="margin-top:6px;color:#94a3b8;font-size:0.75rem;">📋 {av["motivo"]}</div></div><span style="color:#3b82f6;font-size:0.6rem;font-family:monospace;">{tempo_av}</span></div>', unsafe_allow_html=True)
                    if not is_aberta and av["resolvido_em"]: st.markdown(f'<div style="margin-top:4px;color:#00d68f;font-size:0.65rem;">✅ Resolvido em: {av["resolvido_em"]}</div>', unsafe_allow_html=True)
                with col_btns_av:
                    if is_aberta:
                        if st.button("✅", key=f"av_res_{av['id']}"):
                            resolver_avaria(int(av["id"]))
                            st.rerun()
                    if st.button("🗑️", key=f"av_del_{av['id']}"):
    ...
    with t8:
        ...
        def _add_ev(dia, label, tipo="ok"):
            if 1 <= dia <= 31: eventos.setdefault(dia, []).append((label, tipo))

        try:
            for _, av in listar_avarias(apenas_abertas=True).iterrows():
                try:
                    dt = datetime.strptime(av["registrado_em"], "%Y-%m-%d %H:%M:%S")
                    if dt.year == ano and dt.month == mes and dt.date() <= hoje.date(): _add_ev(dt.day, f"Avaria: {av['produto']}", "alerta")
                except: pass
        except: pass
        ...
        try:
            for _, rep in df_reposicao.iterrows():
                try:
                    ...
                    if dt.year == ano and dt.month == mes and dt.date() <= hoje.date(): _add_ev(dt.day, f"Repor: {rep['produto']}", "aviso")
                except: pass
        except: pass

        try:
            for _, h in get_historico_uploads().iterrows():
                try:
                    dt = datetime.strptime(h["data"], "%Y-%m-%d %H:%M:%S")
                    if dt.year == ano and dt.month == mes and dt.date() <= hoje.date(): _add_ev(dt.day, f"Upload: {h['arquivo']}", "info")
                except: pass
        except: pass

        if n_falta > 0 or n_sobra > 0: _add_ev(hoje.day, f"{n_falta} faltas / {n_sobra} sobras", "alerta" if n_falta > 0 else "aviso")

        eventos_js = {str(dia): [{"label": label, "tipo": tipo} for label, tipo in evs] for dia, evs in eventos.items()}
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: transparent; font-family: 'Outfit', sans-serif; color: #e0e6ed; }}
  .wrap {{ display: flex; gap: 20px; align-items: flex-start; flex-wrap: wrap; padding: 4px 2px 8px 2px; }}
  .cal-glass {{ background: rgba(30,41,59,0.55); backdrop-filter: blur(18px); border: 1px solid rgba(255,255,255,0.08); border-radius: 18px; padding: 20px 18px 16px 18px; min-width: 290px; max-width: 320px; box-shadow: 0 8px 32px rgba(0,0,0,0.4); }}
  .cal-header {{ display: flex; justify-content: center; align-items: center; margin-bottom: 14px; }}
  .cal-month {{ font-weight: 700; font-size: 1.05rem; color: #e0e6ed; letter-spacing: .5px; }}
  .cal-grid {{ display: grid; grid-template-columns: repeat(7, 1fr); gap: 3px; text-align: center; }}
  .cal-dow {{ font-family: 'JetBrains Mono', monospace; font-size: 0.52rem; color: rgba(148,163,184,0.55); text-transform: uppercase; letter-spacing: 1px; padding-bottom: 7px; border-bottom: 1px solid rgba(255,255,255,0.05); margin-bottom: 3px; }}
  .cal-day {{ font-size: 0.82rem; color: #cbd5e1; padding: 5px 2px; border-radius: 50%; cursor: pointer; position: relative; transition: background .15s, color .15s, transform .1s; line-height: 1.2; user-select: none; width: 32px; height: 32px; display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 0 auto; }}
  .cal-day:not(.empty):hover {{ background: rgba(0,214,143,0.15); color: #00d68f; transform: scale(1.12); }}
  .cal-day.empty {{ color: transparent; cursor: default; pointer-events: none; }}
  .cal-day.today {{ background: #e0e6ed; color: #0a0f1a; font-weight: 900; }}
  .cal-day.today:hover {{ background: #c8d5e2; color: #0a0f1a; }}
  .cal-day.selected {{ background: rgba(0,214,143,0.25); color: #00d68f; border: 1.5px solid #00d68f; font-weight: 700; }}
  .cal-day.today.selected {{ background: #00d68f; color: #0a0f1a; border: none; }}
  ...
  .dot-ok {{ background: #00d68f; }} .dot-aviso {{ background: #ffa502; }} .dot-alerta{{ background: #ff4757; }} .dot-info {{ background: #3b82f6; }}
  .ev-list {{ flex: 1; min-width: 220px; max-width: 500px; display: flex; flex-direction: column; }}
  .ev-title {{ font-weight: 700; font-size: 0.8rem; color: #94a3b8; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 10px; flex-shrink: 0; }}
  .ev-scroll {{ overflow-y: auto; max-height: 340px; padding-right: 6px; scrollbar-width: thin; scrollbar-color: rgba(0,214,143,0.3) transparent; }}
  .ev-scroll::-webkit-scrollbar {{ width: 4px; }}
  .ev-scroll::-webkit-scrollbar-thumb {{ background: rgba(0,214,143,0.35); border-radius: 4px; }}
  .ev-empty {{ color: #4a5568; font-size: 0.78rem; padding: 10px 0; }}
  .ev-item {{ background: rgba(30,41,59,0.6); border-left: 3px solid #00d68f; border-radius: 0 10px 10px 0; padding: 8px 12px; margin-bottom: 7px; display: flex; flex-direction: column; gap: 2px; animation: fadeIn .2s ease; }}
  @keyframes fadeIn {{ from {{ opacity:0; transform:translateY(4px) }} to {{ opacity:1; transform:translateY(0) }} }}
  .ev-item.alerta {{ border-left-color: #ff4757; }} .ev-item.aviso {{ border-left-color: #ffa502; }} .ev-item.info {{ border-left-color: #3b82f6; }}
...
<body>
<div class="wrap">
  <div class="cal-glass"><div class="cal-header"><span class="cal-month">{nome_mes} {ano}</span></div><div class="cal-grid" id="cal-grid"></div></div>
  <div class="ev-list"><div class="ev-title" id="ev-title">Eventos do mês</div><div class="ev-scroll" id="ev-body"></div></div>
</div>
<script>
  const EVENTOS = {eventos_json}, TOTAL_DIAS = {total_dias}, PRIMEIRO_DIA = {primeiro_dia_semana}, HOJE = {hoje.day}, MES = {mes}, ANO = {ano};
  let diaSel = HOJE;
  function renderCalendario() {{
    const grid = document.getElementById('cal-grid'); grid.innerHTML = '';
    ['D','S','T','Q','Q','S','S'].forEach(d => {{ const el = document.createElement('div'); el.className = 'cal-dow'; el.textContent = d; grid.appendChild(el); }});
    for (let i = 0; i < PRIMEIRO_DIA; i++) {{ const el = document.createElement('div'); el.className = 'cal-day empty'; grid.appendChild(el); }}
    for (let d = 1; d <= TOTAL_DIAS; d++) {{
      const el = document.createElement('div'); const evs = EVENTOS[String(d)] || []; const ds = (PRIMEIRO_DIA + d - 1) % 7;
      let cls = 'cal-day'; if(d===HOJE) cls+=' today'; else if(ds===0||ds===6) cls+=' weekend'; if(d===diaSel) cls+=' selected'; el.className = cls;
      const num = document.createElement('span'); num.textContent = d; el.appendChild(num);
      if(evs.length > 0) {{ const dot = document.createElement('div'); dot.className='dot dot-'+(evs.some(e=>e.tipo==='alerta')?'alerta':(evs.some(e=>e.tipo==='aviso')?'aviso':(evs.some(e=>e.tipo==='info')?'info':'ok'))); el.appendChild(dot); }}
      el.addEventListener('click', () => {{ diaSel = d; renderCalendario(); renderEventos(); }}); grid.appendChild(el);
    }}
  }}
  function renderEventos() {{
    const body = document.getElementById('ev-body'), title = document.getElementById('ev-title');
    let lista = (EVENTOS[String(diaSel)] || []).map(ev => ({{dia: diaSel, ...ev}}));
    title.textContent = lista.length > 0 ? `Dia ${{String(diaSel).padStart(2,'0')}}/${{String(MES).padStart(2,'0')}}/${{ANO}}` : `Sem eventos em ${{String(diaSel).padStart(2,'0')}}/${{String(MES).padStart(2,'0')}}`;
    body.innerHTML = '';
    if(lista.length === 0) {{ body.innerHTML = `<div class="ev-empty">Nenhum evento neste dia.</div>`; return; }}
    lista.forEach(ev => {{ const item = document.createElement('div'); item.className = 'ev-item '+(ev.tipo!=='ok'?ev.tipo:''); item.innerHTML = `<span class="ev-item-title">${{ev.label}}</span><span class="ev-item-sub">${{String(ev.dia).padStart(2,'0')}}/${{String(MES).padStart(2,'0')}}/${{ANO}}</span>`; body.appendChild(item); }});
  }}
  renderCalendario(); renderEventos();
</script>
</body>
</html>
"""
        components.html(cal_html, height=480, scrolling=False)
        st.markdown('<div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:6px;padding-left:2px;"><span style="font-size:0.65rem;color:#64748b;">🟢 Upload &nbsp; 🟡 Reposição &nbsp; 🔴 Avaria &nbsp; 🔵 Info</span></div>', unsafe_allow_html=True)

st.markdown("---")
if _using_cloud: st.markdown('<div class="sync-badge">☁️ CONECTADO AO TURSO · BANCO COMPARTILHADO</div>', unsafe_allow_html=True)
else: st.markdown('<div class="sync-badge">⚠️ MODO LOCAL · Configure TURSO_DATABASE_URL e TURSO_AUTH_TOKEN para compartilhar</div>', unsafe_allow_html=True)
if not has_mestre: st.markdown('<div style="text-align:center;color:#64748b;padding:60px 20px;font-size:1rem;">Faça o upload da planilha mestre acima para começar ☝️</div>', unsafe_allow_html=True)
